...
※ 全500語のうち、画像がまだ無いのは **298語**。ここにはその先頭 **298語** を出しています。

**次に作る5件：finally / improve / consider / suggest / develop**
...
## Lv.2 UNCOMMON（この一覧に1語）

### 1. finally（ついに）　id 260　敵役: orc
...
### 2. improve（改善する）　id 41
例文（日本語）: 若い弓使いが、城の中庭でわらの的を相手に腕を上げていく。  
例文（英語）: A young archer improves her aim at the straw targets in the castle yard.
```
...
### 3. consider（検討する）　id 42　敵役: troll
例文（日本語）: 市場で、腹ぺこのトロルが二つの大きなかぼちゃをじっくり見比べて考える。  
例文（英語）: At the market, a hungry troll considers two big pumpkins carefully.
```
...
### 4. suggest（提案する）　id 43
例文（日本語）: 作戦の天幕で、若い斥候が騎士たちに地図の上の新しい道を提案する。  
例文（英語）: In the war tent, a young scout suggests a new path on the map to the knights.
```
...
### 5. develop（発展する）　id 45
例文（日本語）: 温室で、魔法の種が一晩で光る木へと育つ。  
例文（英語）: In the greenhouse, a magic seed develops into a glowing tree overnight.
```
...
### 6. increase（増える）　id 46　敵役: pirate
例文（日本語）: 海賊たちが船の甲板で箱を空けるたびに、金貨の山が増えていく。  
例文（英語）: The pile of gold increases as the pirates empty their chests on the ship's deck.
```
...
### 7. reduce（減らす）　id 47
例文（日本語）: 農場の庭で、村人たちがバケツの水で納屋の火事を小さな炎にまで弱める。  
例文（英語）: In the farm yard, the villagers reduce the barn fire to small flames with buckets of water.
```
...
### 8. environment（環境）　id 48
例文（日本語）: エルフたちが、森の環境を守るために小川のそばに若木を植える。  
例文（英語）: Elves plant young trees by the stream to protect their forest environment.
```
...
### 9. relationship（関係）　id 49
例文（日本語）: 大広間で、二人の王が握手を交わし、二つの王国の関係を固める。  
例文（英語）: In the great hall, two kings shake hands to seal the relationship between their kingdoms.
```
...
### 10. opportunity（機会）　id 50　敵役: ogre
例文（日本語）: 野営地でオーガがぐっすり眠っているすきに、泥棒は金の鍵を奪う機会をつかむ。  
例文（英語）: With the ogre fast asleep in the camp, the thief sees his opportunity to grab the golden key.
```
...
### 12. effective（効果的な）　id 52　敵役: wraith
...
### 16. describe（描写する）　id 56　敵役: serpent
...
### 17. influence（影響）　id 57　敵役: werewolf
...
### 21. treasure（宝物）　id 261　敵役: pirate
...
### 29. leader（指導者）　id 269　敵役: werewolf
...
### 31. danger（危険）　id 271　敵役: basilisk
...
### 42. evidence（証拠）　id 282　敵役: werewolf
...
### 44. ability（能力）　id 284　敵役: bat, vampire
...
### 45. advantage（利点）　id 285　敵役: ogre
...
### 47. community（地域社会）　id 287　敵役: ogre
...
### 54. destroy（破壊する）　id 294　敵役: golem
...
### 59. gather（集まる）　id 299　敵役: wraith
...
### 63. attract（引きつける）　id 303　敵役: troll
...
### 74. notice（気づく）　id 314　敵役: basilisk
...
### 76. prepare（準備する）　id 316　敵役: werewolf
...
### 77. prevent（防ぐ）　id 317　敵役: goblin
...
### 79. recover（回復する）　id 319　敵役: troll
...
### 85. powerful（強力な）　id 325　敵役: vampire
...
### 86. valuable（貴重な）　id 326　敵役: pirate
...
### 87. curious（好奇心の強い）　id 327　敵役: kobold
...
### 88. enormous（巨大な）　id 328　敵役: troll
...
### 97. actually（実は）　id 337　敵役: shadow
...
### 99. recently（最近）　id 339　敵役: werewolf
...
### 101. implement（実行する）　id 61　敵役: lich
...
### 105. alternative（代わりの）　id 66　敵役: necromancer
...
### 106. consequence（結果）　id 67　敵役: necromancer
...
### 109. strategy（戦略）　id 70　敵役: demon
...
### 111. analyze（分析する）　id 72　敵役: lich
...
### 113. maintain（維持する）　id 74　敵役: fiend
...
### 116. priority（優先事項）　id 77　敵役: lich
...
### 117. comprehensive（包括的な）　id 78　敵役: demon
...
### 119. assumption（前提）　id 80　敵役: lich
...
### 122. contract（契約）　id 343　敵役: demon, warlock
...
### 124. dimension（次元）　id 345　敵役: demon
...
### 130. factor（要因）　id 351　敵役: undead
...
### 132. foundation（土台）　id 353　敵役: demon
...
### 136. identity（正体）　id 357　敵役: vampire
...
### 143. objective（目標）　id 364　敵役: necromancer
...
### 146. procedure（手順）　id 367　敵役: demon
...
### 148. prospect（見通し）　id 369　敵役: demon
...
### 154. allocate（配分する）　id 375　敵役: demon
...
### 155. anticipate（予想する）　id 376　敵役: dread
...
### 157. assess（査定する）　id 378　敵役: fiend
...
### 158. assign（任命する）　id 379　敵役: demon
...
### 160. conclude（結論づける）　id 381　敵役: lich
...
### 161. conduct（実施する）　id 382　敵役: cultist
...
### 162. confirm（確認する）　id 383　敵役: necromancer
...
### 163. consult（相談する）　id 384　敵役: lich
...
### 167. eliminate（取り除く）　id 388　敵役: cultist
...
### 168. emerge（姿を現す）　id 389　敵役: serpent
...
### 170. encounter（出会う）　id 391　敵役: lich
...
### 171. ensure（確実にする）　id 392　敵役: demon
...
### 173. exceed（上回る）　id 394　敵役: demon
...
### 177. indicate（示す）　id 398　敵役: lich
...
### 179. investigate（調査する）　id 400　敵役: necromancer
...
### 184. complex（複雑な）　id 405　敵役: lich
...
### 186. crucial（極めて重要な）　id 407　敵役: demon
...
### 193. ultimate（究極の）　id 414　敵役: lich
...
### 195. vulnerable（傷つきやすい）　id 416　敵役: lich
...
### 197. currently（現在）　id 418　敵役: demon
...
### 198. gradually（徐々に）　id 419　敵役: demon, shadow
...
### 200. leverage（活用する）　id 81　敵役: vampire
...
### 201. mitigate（和らげる）　id 82　敵役: archdemon
...
### 202. acquisition（買収）　id 83　敵役: vampire, cursed
...
### 204. revenue（収益）　id 86　敵役: vampire
...
### 205. disruption（混乱）　id 87　敵役: archdemon
...
### 207. negotiation（交渉）　id 89　敵役: demon
...
### 209. proactively（先手を打って）　id 91　敵役: demon
...
### 210. substantial（かなりの）　id 92　敵役: undead
...
### 216. contingency（不測の事態）　id 98　敵役: lich
...
### 220. alliance（同盟）　id 422　敵役: archdemon
...
### 224. budget（予算）　id 426　敵役: archdemon
...
### 231. demand（需要）　id 433　敵役: vampire
...
### 239. milestone（節目）　id 441　敵役: vampire
...
### 243. profit（利益）　id 445　敵役: pirate
...
### 244. recession（景気後退）　id 446　敵役: demon
...
### 247. supplier（供給業者）　id 449　敵役: undead
...
### 250. warehouse（倉庫）　id 452　敵役: cultist, archdemon
...
### 251. workforce（労働力）　id 453　敵役: titan
...
### 255. acquire（手に入れる）　id 457　敵役: lich
...
### 256. align（足並みをそろえる）　id 458　敵役: archdemon
...
### 259. authorize（認可する）　id 461　敵役: demon
...
### 260. commence（開始する）　id 462　敵役: lich
...
### 261. consolidate（統合する）　id 463　敵役: demon
...
### 265. execute（遂行する）　id 467　敵役: vampire
...
### 266. expedite（早める）　id 468　敵役: undead
...
### 268. forecast（予測する）　id 470　敵役: archdemon
...
### 271. oversee（統括する）　id 473　敵役: demon
...
### 273. procure（調達する）　id 475　敵役: lich
...
### 274. reimburse（払い戻す）　id 476　敵役: demon
...
### 276. restructure（再編する）　id 478　敵役: demon
...
### 279. terminate（終了させる）　id 481　敵役: warlock, archdemon
...
### 283. confidential（機密の）　id 485　敵役: archdemon
...
### 284. flexible（柔軟な）　id 486　敵役: demon
...
### 287. obsolete（時代遅れの）　id 489　敵役: demon
...
### 291. viable（実行可能な）　id 493　敵役: archdemon
...
### 292. temporary（一時的な）　id 494　敵役: demon
...
### 295. accordingly（それに応じて）　id 497　敵役: lich
...
### 297. simultaneously（同時に）　id 499　敵役: archdemon
...
### 298. subsequently（その後）　id 500　敵役: lich
