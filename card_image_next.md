...
※ **COMMON だけ**を図鑑の並び順で出しています。COMMON で画像がまだ無いのは **28語**、ここにはそのうち **28語**。（全レアリティ合わせての残りは 424語）
...
## Lv.1 COMMON（この一覧に28語）

### 1. jump（跳ぶ）　id 150　敵役: slime
例文: A little slime jumps over the stream.　／　小さなスライムが小川をぴょんと跳びこえる。
```
soft hand-painted anime illustration, visible brush texture, matte finish, gentle natural light, warm and clear colors, storybook feel, one clear subject, a simple background that shows the place, few magical effects. Scene: A little slime jumps over the stream. 3:2 wide landscape, the subject is unmistakably the main thing in the picture, important parts kept inside the middle horizontal band, no text, no letters, no logo, no border, no frame
```

### 2. open（開ける）　id 154　敵役: goblin
