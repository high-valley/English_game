...
※ **COMMON だけ**を図鑑の並び順で出しています。COMMON で画像がまだ無いのは **99語**、ここにはそのうち **99語**。（全レアリティ合わせての残りは 499語）
...
## Lv.1 COMMON（この一覧に99語）

### 1. book（本）　id 2　敵役: spider
例文: A spider walks across the big open book.　／　蜘蛛が、大きな開いた本の上を歩いていく。
```
...
### 2. cat（猫）　id 3　敵役: rat
例文: The black cat watches a rat by the wall.　／　黒猫が、壁のそばのネズミをじっと見ている。
```
...
### 3. dog（犬）　id 4　敵役: goblin
例文: The brave dog barks at the goblins.　／　勇敢な犬が、ゴブリンたちに向かって吠える。
```
...
### 4. house（家）　id 5　敵役: goblin
例文: A goblin runs out of the small house.　／　ゴブリンが小さな家から走り出てくる。
```
