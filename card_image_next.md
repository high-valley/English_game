...
※ **COMMON だけ**を図鑑の並び順で出しています。COMMON で画像がまだ無いのは **25語**、ここにはそのうち **25語**。（全レアリティ合わせての残りは 421語）
...
## Lv.1 COMMON（この一覧に25語）

### 1. open（開ける）　id 154
例文: She opens the window to the morning sun.　／　彼女は朝日に向かって窓を開ける。
```
soft hand-painted anime illustration, visible brush texture, matte finish, gentle natural light, warm and clear colors, storybook feel, one clear subject, a simple background that shows the place, few magical effects. Scene: She opens the window to the morning sun. 3:2 wide landscape, the subject is unmistakably the main thing in the picture, important parts kept inside the middle horizontal band, no text, no letters, no logo, no border, no frame
```

### 2. close（閉める）　id 155　敵役: wolf
例文: He closes the door before the wolves come.　／　狼が来る前に、彼はドアを閉める。
```
soft hand-painted anime illustration, visible brush texture, matte finish, gentle natural light, warm and clear colors, storybook feel, one clear subject, a simple background that shows the place, few magical effects. Scene: He closes the door before the wolves come. 3:2 wide landscape, the subject is unmistakably the main thing in the picture, important parts kept inside the middle horizontal band, no text, no letters, no logo, no border, no frame
```
