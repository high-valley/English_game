...
- **レアリティの画像を作り始める前に、そのレアリティの例文をまとめて見直す。** COMMON は1枚ずつ見つけて直したので、`open` / `close` / `jump` などで生成が何度もむだになった。UNCOMMON は生成の前に100語を見直し、64語を書き換えた。RARE も生成の前に100語を見直し、91語を直した（抽象・打ち消し・問いかけの文、ヴァンパイアと人狼の出すぎ、場面の重なり、場所の書き忘れ）
...
- **画像がある単語（`card_art.js` の `CARD_IMG_NAMES`）の例文は変えない**。絵と食い違うため。
  変えるのは絵を作り直すときだけで、そのときは `tools/gen_card_prompts.py` の **`REDO`** に単語を入れる
  （次に作る5件の先頭に「作り直し」として出る）。作り直した絵を登録したら `REDO` から消す
- **蜘蛛を例文に出さない**（ユーザーが苦手。「今後、蜘蛛が出てくるプロンプトはやめて欲しい」）。
  `check_words.py` の `BANNED` が止める。蜘蛛が描かれていた `book` / `big` / `room` は例文を変えて作り直した
...
- **ガチャ画面は、スクロールせずに「引く」ボタンまで見えること。** 部品を足したら、390x844・375x667・430x932 と
  「1回」「まとめて」の両方で、ボタンの下端が下のタブより上にあるかを測る
- **勉強画面も、スクロールせずに答えと結果（正解・例文）まで見えること。** 答えは 2×2 に並べる。
  結果の欄はいちばん長い例文（`culture`）でも伸びない高さを先に取っておく（答えたときに画面がずれない）。
  例文を長くしたら、3つの大きさで結果の欄の下端が下のタブより上にあるかを測る。
  日本語訳（`ja`）はいちばん長くて8文字（足並みをそろえる）。幅375以上なら1行、幅320では2行になるので、
  ボタンは2行でも伸びない高さにしてある。9文字以上の訳を足すときは、全語をボタンに入れて測る
- **勉強とガチャの画面は、スクロールで上下に動かさない**（`showPage` が html に `.noscroll` を付け、`touchmove` を止める）。
  ここに部品を足して1画面からはみ出すと、はみ出した分には届かなくなる。必ず収まる大きさにする。
  カード詳細などの重なり（`main` の外）は止めていない
- **ガチャのパックは、タップで「引く」、左右のスワイプでレベルを変える。** スワイプした直後のクリックで引いてしまわないように、
  `stageTap` が捨てている（`SWIPED_AT`）。パックの部分をいじったら、スワイプでコインが減らないことを確かめる
...
## セーブデータ（重要）
- **保存先（localStorage）に触るのは `js/save.js` の `SAVE_STORE` だけ。** ほかのファイルから `localStorage` を直接読み書きしない。
  App Store 向けにアプリ化したとき、ここだけを差し替えて iCloud などに移すため（SPEC.md §13）
- 読み込んだデータは `initSave()` を通して今の形にそろえる。引き継ぎコードで取り込んだデータも同じ
- **引き継ぎコードの形式（`WG1.…`）を勝手に変えない。** 変えるときは `WG2` にして、`WG1` も読めるようにする。
  Web 版で作ったコードを、将来のアプリ版で読むため
- セーブの形を変えたら、引き継ぎの往復（コードを作る → 別のブラウザで取り込む → 同じ中身になる）を確かめる
...
- **生きものには目と顔を描いてもらう**（`CREATURE`）。目が無いスライムは塊に見える
  ...
  **服を着ていない敵役（トロル・オーガ）には、`FOE_LOOK` に服を書く**（`cross` のトロルが裸の後ろ姿で、お尻が大きく描かれた。子どもも使うアプリなので出せない）。
  ハーピーは羽で胸と腰を覆った姿でよい（ユーザー判断。基準は `follow`）
