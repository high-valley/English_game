...
- **パックの色は、選んでいる排出率のレベルのレアリティの色**（Lv.1 銀・Lv.2 緑・Lv.3 青・Lv.4 紫・Lv.5 赤。Lv.5 は金だと縁取りと見分けにくいので赤）。ガチャ画面と開封演出で同じ色。
  `tools/bake_assets.py` が `gacha_pack` の青い部分だけを塗り替えて5色作る（金の縁取りは残す）。まわりの光もその色にする
- **ガチャ画面は、スクロールせずに「引く」ボタンまで見える**ようにする。パックの大きさは画面の幅と高さの小さいほうに合わせ、
  「まとめて」のときは回数を選ぶ欄のぶん一回り小さくする。高さの低い端末（700px 以下）では説明文を隠す。
  レベルの下の「Lv.○ の排出率を使用中」の表示は、場所を取るので出さない（選んでいるレベルはボタンの光で分かる）
- **勉強画面とガチャ画面は、スクロールせずに1画面に収め、指でなぞっても上下に動かさない**（iPhone の引っぱったときの揺れも止める）。
  勉強画面は答えの4つを 2×2 に並べ、結果（正解・例文）の欄の高さを先に取っておく（答えたときに画面がずれない）。
  高さの低い端末（700px 以下）では「この英単語の意味は？」と熟練度ポイントの行を隠す
- **パックを左右にスワイプしても、排出率のレベルを変えられる**（左へ → 1つ上、右へ → 1つ下。パックが横から入ってくる）。
  パックの左右の矢印（‹ ›）を押しても変わる。端（Lv.1 と、解放したいちばん上のレベル）ではパックが小さく揺れて止まり、矢印は薄くなる。
  パックはタップで「引く」なので、横に40px以上動かしたときだけスワイプとみなし、その直後のタップは引かない
...
| レベル | 主な敵役 |
|---|---|
| Lv.1 COMMON | ゴブリン、スライム、コウモリ、ネズミ、インプ、狼、コボルト、カラス |
| Lv.2 UNCOMMON | オーク、スケルトン、幽霊、山賊、ガーゴイル、ハーピー、ゾンビ、魔女 |
...
- **小カード**（図鑑・ガチャ結果）：同じ枠を縮小し、単語名と星を表示。未入手は暗い枠と「？？？」。3列で表示
  - 小カードには、影やぼかしなどの重い効果を付けない。絵は縮小版を枠に合わせて切り取る（上下が少し切れる）
...
- 旧XPランクは廃止した（`xp` は削除）
- **保存先に触るのは `js/save.js` の `SAVE_STORE` だけ**。ゲーム側は `loadSave` / `writeSave` / `clearSave` を使う。
  App Store 向けにアプリ化したら、`SAVE_STORE` の中身だけを差し替える（§13）
- 読み込んだデータは `initSave()`（`js/app.js`）で今の形にそろえる。引き継ぎコードで取り込んだデータも同じ関数を通す

### データの引き継ぎ（設定画面）
端末ごとにブラウザの中へ保存しているので、別の端末・別のブラウザでは最初からになる。そのための写し。
- 設定画面の「データの引き継ぎ」で、**引き継ぎリンク**（共有ボタン。無ければコピー）と**引き継ぎコード**（コピー）を出す
- 受け取った端末では、リンクを開くか、設定画面にコードかリンクを貼り付けて「取り込む」
- 取り込む前に「取り込むデータ」と「この端末の今のデータ」（コイン／カード種類／連続学習）を並べて確認を出す。上書きすると元に戻せない
- 自動の同期ではない。2台で交互に遊ぶときは、切り替えるたびに送り直す
- iPhone の Safari は、ホーム画面に追加していないサイトを7日間開かないと保存データを消すことがある。控えとしても使える
- コードの形式：`WG1.<z|p>.<本体>.<検査>`
  - `WG1`：形式の版。中身の形を変えるときは `WG2` にし、古い版のコードも読めるようにしておく（アプリ版で Web 版のコードを読むため）
  - `z`：本体を deflate-raw で圧縮（`CompressionStream`）。使えない環境では `p`（圧縮なし）
  - 本体：セーブデータの JSON を base64url にしたもの。検査：圧縮前の JSON の CRC32（16進8桁）
  - 500語すべてを持っていても約3,400文字（圧縮あり）
- リンクは `<サイトの URL>#import=<コード>`。`#` より後はサーバーに送られない。開いたらすぐ `#` を消す
- 途中で切れた・書き換わった・新しい版のコードは、理由を日本語で出して取り込まない
- **開発者用の「全部カンスト」データ**：`python3 tools/dev_save.py` がコードを作り、`dev_save_code.md` に書く。
  全語を各99枚・熟練度最大・解放 Lv.5・全クリア表示済み・コイン 9,999,999・連続学習 999日。
  連続学習は作った日の翌日まで有効なので、それより後に使うときは作り直す。単語を足したり消したりしたときも作り直す
...
- 更新ボタンで新しい版を開いたとき（と、新しい版を見つけたとき）は、ふだん開く `index.html` のキャッシュも新しくする（`refreshStartCache`）。
  そうしないと、閉じて開き直したときに、最大10分、古い版に戻る
- **背景は、日本時間で昼と夜を切り替える**：6〜18時は `<置き場>_day`（昼）、18〜6時は今までの絵（夜）。
  ホーム・勉強・ガチャ（開封演出も）・図鑑の4画面（`home_bg_day` / `study_bg_day` / `gacha_bg_day` / `cards_bg_day`）。起動画面は夜のまま。
  端末の時計の地域に関係なく日本時間で決める（`ui_images.js` の `timeSlot`）。昼の絵が無い画面は、昼も夜の絵。
  開いたまま6時・18時をまたいだら、今の画面の背景を、1分以内（またはアプリに戻ってきたとき）に1.2秒かけて入れ替える。
  昼はホームのキャッチコピーの影を濃くする
...
- **ガチャ**：現在のレベルの排出率を表示
- **カード**：レアリティ別の図鑑。**絞り込みボタンに、レア度ごとの解放数（所持している種類 / そのレア度の全種類）を出す**。スクロールで全件表示（ページ送りは廃止）。はじめに24枚を並べ、下に近づくと24枚ずつ足す（500枚を一度に作ると重く、iPhone で落ちたため）。未入手は「？？？」。詳細は発音・例文・熟練度を表示
...
│   ├─ dev_save.py          開発者用の「全部カンスト」セーブの引き継ぎコードを作る（dev_save_code.md）
...
│   ├─ save.js             セーブの保存先（ここだけが触る）と、引き継ぎコードの作成・読み込み
│   ├─ app.js              ホーム・カード図鑑・設定（引き継ぎ）・画面切り替え
...
## 13. 将来
### App Store でのリリース（今はお金をかけない）
- 今の HTML・JS を **Capacitor** で包んで iPhone アプリにする（作り直さない）
- アプリの中の保存場所は Safari とは別になる。**Web 版からアプリ版へは、引き継ぎコード（§8）で移す**。そのため形式の版（`WG1`）を守る
- 端末どうしの自動同期は **iCloud のキー値ストア**を使う（費用なし・ログイン不要・1MBまで。セーブは数KB）。`SAVE_STORE` を差し替えて実装する
- **自前のアカウント（Firebase などのログイン）は使わない**。App Store では、他社ログインを入れると「Apple でサインイン」も必要になり、
  アカウントを作れるならアプリ内での削除も必要になる。預かる個人データも増える
- お金がかかるところ：Apple Developer Program の年会費（米国では年99ドル）、組み立て用の Mac（Xcode）
- 審査では、Web サイトを包んだだけのアプリは断られることがある。オフラインで遊べる、連続学習の通知など、アプリらしい機能を足す
