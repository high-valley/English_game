...
6. 各レアリティ20語（合計100語）から始め、**今後、各レアリティの単語を増やす**
...
- 排出率は、**到達した最高レベル**の行を使う。レベルが上がると更新される
- 確率は、設定（`GACHA_RATES`）にまとめる
- 抽選されたレアリティの20語から、ランダムに1枚
...
- 熟練度は、勉強で上がった値（0〜5）
- 画像の差し替え：`assets/cards/単語.webp`（3:2）＋ `card_art.js` の `CARD_IMG`。プロンプトは `card_image_prompts.md`

---

## 8. セーブデータ（localStorage、キー `wordQuestDemo` は継続）
```
{ coins, owned:{id:枚数}, mastery:{id:0〜5}, unlockedLevel:1〜5, studyLevel, streak, last, v:2 }
```
- レベルの進捗率は、`mastery` から計算する（保存しない）
- `unlockedLevel` は増えるだけ。「クリア済み」は、`unlockedLevel` から求める
- IDの考え方は §6。単語を消してもセーブが壊れない
- **v2への切り替え（実施済み）**：単語を入れ替えたので、カードと熟練度をリセットした。コインと連続学習は引き継ぐ（テスト版のため）
- `xp` は、旧XPランクの廃止（段階4）で削除する

---

## 9. 画面
- **下のタブ：ホーム／勉強／ガチャ／カード**（復習は削除する）
...
```
index.html
css/   style.css / upgrade.css（追加・上書き）
js/    words.js / app.js / card_art.js / ui_images.js / gacha_ui.js
assets/ ui/（背景・枠・ロゴ・アイコン）  cards/（単語ごとの画像）
SPEC.md / TODO.md / card_image_prompts.md
```
...
| 3 | 復習の削除（タブ4つ）、熟練度は勉強だけで上げる | 未 |
| 4 | レベル制の勉強（進捗、切り替え、クリア）、XPランク廃止 | 未 |
| 5 | レベルに応じたガチャ排出率（`GACHA_RATES`） | 未 |
| 6 | ホームに学習レベルと進捗、ロック表示 | 未 |
| 7 | レア度別の枠、図鑑の整理、小カードの新デザイン | 未 |
| 8 | レベルごとのコイン増（`LEVEL_COIN`） | 将来 |

※段階2〜3の間は、勉強がLv.1の単語だけ、復習が残っている。3と4は続けて行うのがおすすめ
