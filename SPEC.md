...
- **ホームの背景は、日本時間で昼と夜を切り替える**：6〜18時は `home_bg_day`（昼）、18〜6時は `home_bg`（夜）。
  端末の時計の地域に関係なく日本時間で決める。昼の絵が無ければ、いつも夜の絵。
  開いたまま6時・18時をまたいだら、ホームにいるときだけ、1分以内（またはアプリに戻ってきたとき）に1.2秒かけて入れ替える。
  ほかの画面にいるときは、ホームに戻ったときに正しい絵を出す。昼はキャッチコピーの影を濃くする
