...
## 次のタスク（優先順）
1. **カード画像の作成（COMMON から）**。`card_image_next.md` に COMMON の残り82語が図鑑の並び順で入っている
   （COMMON は 18/100。ほかに UNCOMMON / RARE / EPIC / LEGENDARY が各1枚＝全体 22/500）
   - 画像の生成は画像生成AI（ChatGPT / Grok など）に頼む（Claude は画像を作れない）
   - **プロンプトを渡すときは、必ず `card_image_prompts.md` か `gen_card_prompts.py` から引く。**
     例文を記憶で書くと、絵と例文が食い違う（`run` の例文を取り違えて1枚むだにした）
...
## 候補（今後）
- **先に作った7枚を、今の画風で作り直すか**（apple / book / cat / family / food / friend / school）。
  この7枚はプロンプトを直す前の絵柄（濃いめのアニメ塗り）で、今の指定は柔らかい手描き。
  COMMON が100枚そろうころには、この7枚が少数派になる
- **`house` と `friend` を作り直す**（上の7枚のうちの2枚）。ゴブリンが旧デザインのままで、
  基準（`FOE_LOOK` の「痩せて険しい緑の男」＝いまの `dog` / `go`）と揃っていない。
  ゴブリンが出るカードで姿が食い違って見えるので、7枚の中では優先度が高い
