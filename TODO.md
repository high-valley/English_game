...
3. **勉強データを確認できる画面**（ユーザーの希望）
   ...
   - **正答率・苦手な単語・日ごとの学習量**を出すには、セーブに新しい記録が要る
     （例：単語ごとの正解・不正解の回数、日ごとの問題数）。記録は足したその日から数えるので、早めに入れるほど後で役に立つ
...
4. **品詞（名詞・動詞・形容詞・副詞）で分けて勉強できる仕組み**（ユーザーの希望）
   ...
   - **副詞がとても少ない**：COMMON 0語・UNCOMMON 6語・RARE 4語・EPIC 4語・LEGENDARY 5語。
     ...
     レベルをまたいで品詞だけで出すか、副詞だけは全レベルまとめにするかを決める
   - 4択の選択肢（`makeOptions`）を同じ品詞からも選ぶか。同じ品詞にすると、見た目で答えが絞れず、少し難しくなる
   - 品詞で勉強した分も、熟練度（`mastery`）・レベルの進み具合・コインに数えるか。数えるなら、今の勉強と同じ `answerCorrect` を通す
   - 勉強画面は1画面に収めている（CLAUDE.md）。品詞の切り替えを足すなら、レベルのボタンの行に入れるなど、高さを増やさない形にし、
     390x844・375x667・430x932 で結果の欄まで収まるかを測る
...
- **起動と図鑑の読み込みを速くした**（キャッシュ無し・4G・スマホ相当の CPU で実測）。本物の背景が出るまで 約7.5秒 → 約1.9秒、起動時の転送 12.1MB → 1.9MB、図鑑を開いたときの転送 14.5MB → 0.8MB。旧背景（コードで描いた夜の城）が一瞬出る問題も解消。`tools/bake_assets.py` で枠・アイコンの透過と図鑑用の縮小版を前もって作り、画像はファイルごとの版にした（カードを足しても、他の画像は再ダウンロードされない）
- 起動画面が「TAP TO START」なのに2.6秒で勝手に閉じていたのを、タップするまで閉じないようにした
