...
const currentRates=()=>{const R=GACHA_RATES[S.unlockedLevel]||GACHA_RATES[1];return LEVELS.map(k=>[k,R[k]])};
// クリアしたレベルの次を解放する。解放したレベル（無ければ0）と、全レベルクリアの初回かどうかを返す
function syncLevels(){
  let up=0,all=false;
  for(let lv=1;lv<LEVELS.length;lv++)if(levelPercent(lv)>=100&&S.unlockedLevel<=lv){S.unlockedLevel=lv+1;up=lv+1}
